...
highlight clear
syntax reset
let g:colors_name="{g:colors_name}"

" Color definitions
let s:t0="{s:gui00}" " Default Background
let s:t1="{s:gui01}" " Lighter Background (Status bars)
let s:t2="{s:gui02}" " Selection Background
let s:t3="{s:gui03}" " Comments, Invisibles
let s:t4="{s:gui04}" " Dark Foreground (Status bars)
let s:t5="{s:gui05}" " Default Foreground
let s:t6="{s:gui06}" " Light Foreground (Rarely used)
let s:t7="{s:gui07}" " Light Background (Rarely used)
let s:t8="{s:gui08}" " Debug, Identifiers, Diff Delete
let s:t9="{s:gui09}" " Numbers, Booleans, Constants
let s:tA="{s:gui0A}" " Classes, Labels
let s:tB="{s:gui0B}" " Strings, Diff Add
let s:tC="{s:gui0C}" " Escapes, RegExp
let s:tD="{s:gui0D}" " Function, Include, Directory
let s:tE="{s:gui0E}" " Keywords, Storage, Diff Change
let s:tF="{s:gui0F}" " Delimiters, SpecialChar
